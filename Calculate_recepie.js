    ...
    for (let [key, value] of Object.entries(object)) {
        if (Array.isArray(object[key])) {
            for (let item in object[key]) {
                object[key][item] = func(object[key][item], location + key + "|" + item);
            }
...
const setup_func = () => {
    ...
    const set_ingredient_nutrition_values = (val) => ingredient_nutrition_values = val;
...
for (let item of recepy["Ingredient List"]) {
    let using = undefined;
    const test = (ingredients_info.filter(element => element.ingredient === item.Ingredient));
    if (test.length === 0) {
        ...
        const find_by_brand = (test.filter(element => element.brand === item.brand));
        if (find_by_brand === 1) {
            using = find_by_brand[0];
        } else if (find_by_brand > 1) {
            using = find_by_brand[0];
        } else {
            const find_generic = (test.filter(element => element.brand === "Generic"));
    ...
    if (!!using) {
        ...
        func_obj(total_nutrition, multiplier_func.calculate_nutrition_from_ingredient_value);
